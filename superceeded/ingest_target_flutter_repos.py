...
from typing import List, Dict, Optional # Add Optional too, as we might use it
...
DART_PARSER_SCRIPT_PATH = "/mnt/caseSSD/mcp_server_project/dart_ast_parser.dart" # Path to your Dart parser
...
REPOS_TO_PROCESS = {
    ...
    # Uncomment and adjust target_subdirs when ready for a fuller ingestion.
     "flutter_sdk": {
         ...
         "base_metadata": {"package_group": "flutter_sdk_examples", "language": "dart", "framework": "flutter"}
     }
}
...
def generate_chunk_id_from_parsed(original_filepath: str, element_name: str, element_type: str, chunk_index_in_file: int) -> str:
...
def call_dart_parser(dart_file_path: str) -> List[Dict]:
    ...
    try:
        ...
        if process.returncode != 0:
            logger.error(f"Dart parser script failed for {dart_file_path}. Return code: {process.returncode}")
            logger.error(f"Dart parser stderr: {process.stderr.strip() if process.stderr else 'N/A'}")
            ...
            for line in process.stdout.strip().split('\n'):
                if line.strip():
                    try:
                        ...
                        logger.error(f"Failed to decode JSON from Dart parser for file {dart_file_path}: {je} - Line: '{line}'")
            logger.debug(f"Dart parser returned {len(parsed_elements)} elements for {dart_file_path}")
        else:
            logger.warning(f"Dart parser returned no stdout for {dart_file_path}. Stderr: {process.stderr.strip() if process.stderr else 'N/A'}")
            
    except subprocess.TimeoutExpired:
        logger.error(f"Dart parser script timed out for {dart_file_path} after {DART_SCRIPT_TIMEOUT} seconds.")
    except Exception as e:
        logger.error(f"Error calling Dart AST parser for {dart_file_path}: {e}", exc_info=True)
...
def prepare_documents_from_parsed_elements(
    ...
    """
    chroma_docs = []
    relative_path_in_repo = os.path.relpath(original_dart_filepath, start=base_repo_path)
    ...
    for i, element_data in enumerate(parsed_elements):
        ...
        if not code_block.strip() or len(code_block.strip()) < MIN_CHUNK_CODE_LENGTH:
            logger.debug(f"Skipping very short or empty code block for {element_name} in {relative_path_in_repo}")
            continue
        ...
        metadata = {
            **common_repo_metadata,
            ...
            "doc_comment_summary": doc_comment[:200] + ('...' if len(doc_comment) > 200 else ''), # Store a summary
        ...
        doc_id = generate_chunk_id_from_parsed(original_dart_filepath, element_name, element_type, i)
...
def send_batch_to_mcp(batch: List[Dict]):
    ...
    payload = {"documents": documents_to_send, "metadatas": metadatas_to_send, "ids": ids_to_send}
    try:
        logger.info(f"Sending batch of {len(batch)} AST-parsed chunks to MCP... First ID: {ids_to_send[0] if ids_to_send else 'N/A'}")
        response = requests.post(MCP_ADD_KNOWLEDGE_URL, json=payload, timeout=REQUEST_TIMEOUT)
        response.raise_for_status()
        response_json = response.json()
        if response_json.get("status") == "success":
            res_result = response_json.get('result', {})
            logger.info(f"MCP Add Knowledge Response: {res_result.get('message', 'Success')}")
        else:
            err_msg = response_json.get('error_message', 'Unknown error from MCP')
            logger.error(f"MCP Add Knowledge Failed: {err_msg}")
            if response_json.get('result') and isinstance(response_json.get('result'), dict) and response_json['result'].get('error'):
...
def main():
    ...
    if not os.path.isfile(DART_PARSER_SCRIPT_PATH):
        logger.critical(f"Dart AST parser script not found at: {DART_PARSER_SCRIPT_PATH}. Exiting.")
        return
    if not os.path.isdir(CLONED_REPOS_BASE_PATH):
        logger.critical(f"Base path for cloned repos not found: {CLONED_REPOS_BASE_PATH}. Exiting.")
        return
    ...
    for repo_folder_name, repo_config in REPOS_TO_PROCESS.items():
        ...
        if not os.path.isdir(repo_full_path):
            logger.warning(f"Repository directory not found, skipping: {repo_full_path}")
            continue
        ...
        for target_subdir in repo_config.get("target_subdirs", []):
            ...
            if not os.path.isdir(current_processing_path):
                logger.warning(f"Target subdirectory not found in '{repo_folder_name}', skipping: {current_processing_path}")
                continue
            ...
            if document_batch:
                logger.info(f"Sending remaining batch for subdir '{target_subdir}' in '{repo_folder_name}'...")
                send_batch_to_mcp(document_batch)
                document_batch = []
                time.sleep(0.5)
...
if __name__ == "__main__":
    logger.info("This script (V2 AST-based) will use dart_ast_parser.dart to chunk .dart files")
    ...
    try:
        ...
        logger.critical(f"An unhandled error occurred in the main V2 ingestion script: {e}", exc_info=True)