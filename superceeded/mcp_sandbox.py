...
def setup_sandbox_environment() -> str:
    ...
    try:
        ...
        temp_dir = tempfile.mkdtemp(
            prefix=SandboxConfig.WORKING_DIR_PREFIX, dir=base_dir
        )
        return temp_dir
...
def execute_code_subprocess(code: str, working_dir: str) -> Dict[str, Any]:
    ...
    try:
        ...
        process = subprocess.Popen(
            ...
            preexec_fn=lambda: (
                resource.setrlimit(
                    resource.RLIMIT_AS, (SandboxConfig.MAX_MEMORY_MB * 1024 * 1024, -1)
                )
        ...
        if len(stdout) > SandboxConfig.MAX_OUTPUT_SIZE:
            stdout = (
                stdout[: SandboxConfig.MAX_OUTPUT_SIZE] + "\n... (output truncated)"
            )

        if len(stderr) > SandboxConfig.MAX_OUTPUT_SIZE:
            stderr = (
                stderr[: SandboxConfig.MAX_OUTPUT_SIZE]
...
def execute_code(code: str, method: str = "auto") -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"{status} Sandbox execution completed in {result['execution_time']:.2f}s using {result['method']}"
        )
...
if __name__ == "__main__":
    ...
    for i, code in enumerate(test_codes, 1):
        ...
        status = "" if result["success"] else ""
        print(
            f"{status} Result: {result['stdout'].strip() if result['stdout'] else result['stderr']}"
        )
...
class SandboxExecuteSchema(BaseModel):
    code: str = Field(
        description="Python code to execute in the secure sandbox environment. Must be valid Python syntax."
    )


class SandboxDebugSchema(BaseModel):
    code: str = Field(
        description="Python code to debug in the sandbox with detailed execution analysis."
    )
    expected_output: Optional[str] = Field(
        description="Expected output for comparison", default=None
    )
...
class LangchainSandboxExecuteTool(LangchainBaseTool):
    ...
    def _run(self, code: str) -> str:
        ...
        try:
            ...
            if result.get("validation", {}).get("warnings"):
                output_parts.append(
                    f"Validation warnings: {'; '.join(result['validation']['warnings'])}"
                )
...
class LangchainSandboxDebugTool(LangchainBaseTool):
    ...
    async def _arun(self, code: str, expected_output: Optional[str] = None) -> str:
        return self._run(code, expected_output)


class LangchainSandboxStatsTool(LangchainBaseTool):
    name: str = "get_sandbox_stats"
    description: str = (
        "Gets diagnostic information about the sandbox system status, configuration, and health."
    )
    ...
    def _run(self) -> str:
        ...
        try:
            ...
            if result["status"] == "success":
                ...
                summary += f"• Available: {stats.get('sandbox_available', 'unknown')}\n"
                summary += f"• Subprocess support: {stats.get('subprocess_available', 'unknown')}\n"
                summary += f"• Timeout: {stats.get('timeout_seconds', 'unknown')}s\n"
                summary += f"• Max output size: {stats.get('max_output_size', 'unknown')} chars\n"
                summary += f"• Max memory: {stats.get('max_memory_mb', 'unknown')} MB\n"
                summary += (
                    f"• Test execution: {'✓' if stats.get('test_execution') else '✗'}\n"
                )
                summary += (
                    f"• Base directory: {stats.get('base_directory', 'unknown')}\n"
                )
                summary += f"• Security: {stats.get('allowed_imports_count', 0)} allowed, {stats.get('blocked_imports_count', 0)} blocked imports"
                return summary
            else:
