...
def step1_retrieve_memories(
    ...
    try:
        ...
        # Generate embedding for the query using nomic-embed-text (only once)
        query_embedding_response = ollama.embeddings(
            model=MEMORY_EMBEDDING_MODEL, prompt=query
        )
...
def health_check_3step_memory() -> Dict[str, Any]:
    ...
    try:
        ...
        test_embedding = ollama.embeddings(model=MEMORY_EMBEDDING_MODEL, prompt="test")
        if test_embedding and "embedding" in test_embedding:
            health_status["embedding_model_available"] = True
            logger.info(f"Embedding model {MEMORY_EMBEDDING_MODEL} health check: PASS")

    except Exception as e:
        health_status["errors"].append(
            f"Embedding model {MEMORY_EMBEDDING_MODEL} failed: {e}"
        )
        ...
        if existing_data:
            ...
            try:
                result = ollama.embeddings(
                    model=MEMORY_EMBEDDING_MODEL, prompt=conversation_text
                )
                conversation_data["embedding"] = result["embedding"]
                logger.debug(f"Updated embedding for conversation: {conversation_id}")
            except Exception as e:
                logger.warning(f"Failed to update embedding for {conversation_id}: {e}")
...
async def orchestrate_request(
    ...
    # Use direct ChatOllama without tool binding for performance
    llm = ChatOllama(
    ...
    # --- 2. Get Memory Context using fast architecture ---
    memory_context_text = ""
    try:
        # Use fast memory architecture for instant context retrieval
        if redis_client:
            logger.info("Using fast memory architecture for context retrieval")
            memory_context_text = get_fast_memory_context(redis_client, user_message)
        else:
            logger.warning("Redis client not available, skipping memory context")
            if MEMORY_FALLBACK_DISABLED:
                raise Exception("FAIL FAST: Redis unavailable and fallback disabled")

    except Exception as e:
        logger.error(f"Fast memory context retrieval error: {e}")
        if MEMORY_FALLBACK_DISABLED:
            raise Exception(f"FAIL FAST: Memory context retrieval failed: {e}")
        # If fallback is enabled, continue with empty context
        memory_context_text = ""
    ...
    system_prompt = (
        ...
        # Step 2: User rules from MongoDB (permanent)
        f"{user_rules}\n"
        # Step 3: Memory context (dynamic)
    ...
    try:
        ...
        # Get direct response from LLM without tool binding
        ...
        if isinstance(response, AIMessage) and response.tool_calls:
            ...
            final_response_message = await llm_with_tools.ainvoke(lc_messages)
            ...
            final_response = strip_thoughts_from_content(final_processed_content)
            ...
            if redis_client:
                try:
                    ...
                    save_conversation_to_redis(
                        redis_client, conversation_id, new_messages
                    )
                except Exception as e:
                    logger.error(f"Failed to save conversation {conversation_id}: {e}")
        ...
        # FIX: Ensure the final content is always a string and strip thinking blocks
        final_response = strip_thoughts_from_content(processed_content)
