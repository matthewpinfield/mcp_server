    ...
    try:
        if action == "save_rule":
            ...
            if result.get("status") == "success":
                ...
                return f"[PASS] Rule added: {args}\nRule ID: {rule_id}"
            else:
                return (
                    f"[FAIL] Failed to add rule: {result.get('error', 'Unknown error')}"
                )
            ...
            if result.get("status") == "success":
                return f"[PASS] Information saved: {args}"
            else:
                return f"[FAIL] Failed to save: {result.get('error', 'Unknown error')}"

        elif action == "get_memory":
            result = mcp_get_context(args, include_long_term=True)
            if result.get("status") == "success":
                context = result.get("context", {})
                response = f"[INFO] Memory search results for '{args}':\n"
                response += (
                    f"• {len(context.get('short_term', []))} recent interactions\n"
                )
                response += (
                    f"• {len(context.get('profile', {}).get('rules', []))} rules\n"
                )
                response += f"• {len(context.get('long_term', []))} semantic matches"
                return response
            else:
                return f"[FAIL] Memory search failed: {result.get('error', 'Unknown error')}"

        elif action == "correct" or action == "fix":
            result = mcp_add_correction(args)
            if result.get("status") == "success":
                return f"[PASS] Correction saved: {args}"
            else:
                return f"[FAIL] Failed to save correction: {result.get('error', 'Unknown error')}"
            ...
            if result.get("status") == "success":
                ...
                if not rules:
                    return f"[INFO] No rules found{' matching \"' + args + '\"' if args.strip() else ''}"
                response = f"[INFO] Found {len(rules)} rule(s):\n"
                ...
                return f"[FAIL] Failed to list rules: {result.get('error', 'Unknown error')}"

        elif action == "delete_rule":
            if not args.strip():
                return "[FAIL] Please provide a rule ID to delete. Usage: /delete_rule <rule_id>"
            result = mcp_delete_rule(args.strip())
            if result.get("status") == "success":
                return f"[PASS] Rule {args.strip()} deleted successfully"
            else:
                return f"[FAIL] Failed to delete rule: {result.get('error', 'Unknown error')}"

        elif action == "change_rule":
            parts = args.split(" ", 1)
            if len(parts) < 2:
                return "[FAIL] Please provide rule ID and new text. Usage: /change_rule <rule_id> <new_text>"
            rule_id, new_text = parts
            result = mcp_update_rule(rule_id.strip(), new_text.strip())
            if result.get("status") == "success":
                return f"[PASS] Rule {rule_id} updated successfully"
            else:
                return f"[FAIL] Failed to update rule: {result.get('error', 'Unknown error')}"
            ...
            if category:
                commands = {
                    k: v
                    for k, v in DEFAULT_SLASH_COMMANDS.items()
...
class MemorySystem:
    ...
    def _setup_chromadb_tier3(self):
        ...
        try:
                    ...
                    def __call__(self, input: Documents) -> Embeddings:
                        ...
                        for text in input:
                            try:
                                ...
                                logger.error(
                                    f"Ollama embedding error for text '{text[:50]}...': {e}"
                                )
            ...
            self.tier3_memory = self.chroma_tier3_client.get_or_create_collection(
                name="tier3_memory", embedding_function=self.embedding_func
            )

            logger.debug(
                f"ChromaDB Tier 3 initialized with {self.tier3_memory.count()} memories"
            )
        except Exception as e:
    ...
    def _setup_chromadb_nas(self):
        ...
        try:
            if not self._check_and_mount_nas():
                logger.warning(
                    "NAS not available - Tier 3b (long-term archive) will be limited"
                )
                self.chroma_nas_client = None
            ...
            try:
                ...
                logger.warning(
                    f"NAS write access failed: {e} - Tier 3b will be read-only"
                )
            ...
            self.nas_archive = self.chroma_nas_client.get_or_create_collection(
                name="nas_archive", embedding_function=self.embedding_func
            )

            logger.debug(
                f"ChromaDB NAS archive initialized with {self.nas_archive.count()} archived memories"
            )
        except Exception as e:
            logger.warning(
                f"ChromaDB NAS connection failed: {e} - long-term archive unavailable"
            )
            self.chroma_nas_client = None
            self.nas_archive = None

    def save_interaction(
        self, messages: List[Dict], tags: Optional[Dict] = None
    ) -> Dict:
        ...
        try:
            ...
            if self.redis_client is not None:
                # Compute embedding for semantic search (save-time cost for fast retrieval)
                embedding = None
                if conversation_text.strip():
                    try:
                        result = ollama.embeddings(
                            model=DEFAULT_EMBEDDING_MODEL, prompt=conversation_text
                        )
                        embedding = result["embedding"]
                        logger.debug(
                            f"Computed embedding for interaction: {interaction_id}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"Failed to compute embedding for {interaction_id}: {e}"
                        )

                conversation_data = {
                    "text": conversation_text,
                    "embedding": embedding,  # Store for fast semantic search
                    "metadata": metadata,
                ...
                logger.info(
                    f"Saved to Tier 1 (Redis) for 14-day migration: {interaction_id}"
                )
                return {
                    "status": "success",
                    "interaction_id": interaction_id,
    ...
    def migrate_tier1_to_tier3(self) -> Dict:
        ...
        try:
            if self.redis_client is None or self.tier3_memory is None:
                return {
                    "status": "error",
                    "error": "Redis or ChromaDB unavailable for migration",
                }
            ...
            for key in interaction_keys:
                try:
                    ...
                    if ttl <= 86400:  # Less than 1 day left, migrate to Tier 3
                        ...
                        ttl == -1
                    ):  # Key has no expiration, shouldn't happen but clean up
            ...
            logger.info(
                f"Migration complete: {migrated_count} conversations moved to Tier 3"
            )
            return {
                "status": "success",
    ...
    def migrate_tier3_ssd_to_nas(
        self, nas_chromadb_path: str = "/mnt/my_nas_mcp_share/archives/memory_vector_db"
    ) -> Dict:
        ...
        try:
            if self.tier3_memory is None:
                return {
                    "status": "error",
                    "error": "ChromaDB Tier 3a (SSD) unavailable for NAS migration",
                }
                    ...
                    if conversation_timestamp:
                        try:
                            ...
                            if isinstance(conversation_timestamp, str):
                                conv_dt = datetime.fromisoformat(
                                    conversation_timestamp.replace("Z", "+00:00")
                                )
                            ...
                            if conv_ts < cutoff_timestamp:
                                old_conversations["documents"].append(
                                    all_results["documents"][i]
                                )
                                old_conversations["metadatas"].append(
                                    all_results["metadatas"][i]
                                )
                                old_conversations["ids"].append(conversation_id)
                        except (ValueError, TypeError) as e:
                            logger.warning(
                                f"Invalid timestamp format for {conversation_id}: {e}"
                            )
                            continue
            ...
            if not results["ids"]:
                logger.info(
                    "No conversations older than 30 days found for NAS migration"
                )
                return {
                    "status": "success",
                    "migrated_count": 0,
            ...
            logger.info(
                f"Migrated {migrated_count} conversations from Tier 3a (SSD) to Tier 3b (NAS): {nas_chromadb_path}"
            )
    ...
    def _get_redis_context(self, query: Optional[str] = None) -> List[Dict]:
        """Retrieve conversation context from Redis using semantic search"""
        ...
        try:
            ...
            return self._search_redis_semantic(query)

        except Exception as e:
            logger.error(f"Failed to get Redis context: {e}")
            return []

    def _search_redis_semantic(self, query: str, limit: int = 10) -> List[Dict]:
        """Semantic search on Redis conversations using pre-computed embeddings"""
        if self.redis_client is None:
            return []
        ...
        try:
            ...
            try:
                result = ollama.embeddings(model=DEFAULT_EMBEDDING_MODEL, prompt=query)
            ...
            for key in cast(list, keys):
                ...
                if data:
                    try:
                        interaction_data = json.loads(cast(str, data))
                        stored_embedding = interaction_data.get("embedding")

                        # Skip conversations without embeddings (old format)
                        ...
                        logger.debug(f"Error processing conversation {key}: {e}")
                        continue

            # Sort by similarity and return top results
            results.sort(key=lambda x: x["similarity"], reverse=True)

            # Remove similarity from final results
            for result in results:
                result.pop("similarity", None)

            logger.debug(
                f"Redis semantic search found {len(results)} results for query: {query}"
            )
            return results[:limit]

        except Exception as e:
            logger.error(f"Failed to search Redis semantically: {e}")
            return []
    ...
    def _search_long_term_memory(self, query: str, n_results: int = 5) -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                nas_client = chromadb.PersistentClient(
                    path="/mnt/my_nas_mcp_share/archives/memory_vector_db"
                )
                ...
                if nas_results and nas_results.get("documents"):
                    ...
                    if nas_documents and len(nas_documents) > 0 and nas_documents[0]:
                        for i, doc in enumerate(nas_documents[0]):
                            metadata = (
                                nas_metadatas[0][i]
                                if nas_metadatas and len(nas_metadatas[0]) > i
                                else {}
                            )
                            distance = (
                                nas_distances[0][i]
                                if nas_distances and len(nas_distances[0]) > i
                                else 0.0
                            )
...
def mcp_migrate_tier3_to_nas(
    nas_chromadb_path: str = "/mnt/my_nas_mcp_share/archives/memory_vector_db",
) -> Dict:
...
class MemorySaveSchema(BaseModel):
    ...
    interaction_summary: Optional[str] = Field(
        None,
        description="A brief text summary of the interaction to be saved. Use this for simple notes.",
    )
    messages: Optional[List[Dict[str, str]]] = Field(
        None, description="The full list of chat messages to save for complete context."
    )
    tags: Optional[Any] = Field(
        None,
        description="Tags for categorization. Can be a simple list of strings ['tag1', 'tag2'] or a dictionary {'key': 'value'}.",
    )
    ...
    def check_one_content_field(cls, self):
        if not self.interaction_summary and not self.messages:
            raise ValueError(
                'Either "interaction_summary" or "messages" must be provided to save to memory.'
            )
        return self
...
class MemoryRuleSchema(BaseModel):
    rule: str = Field(
        description="The rule or preference text. Example: 'I prefer detailed explanations' or 'Always use TypeScript for web projects'"
    )
    category: str = Field(
        description="Category like 'preference', 'coding_style', 'communication', etc.",
        default="preference",
    )
...
class LangchainMemorySaveTool(AsyncTool):
    ...
    def _run(
        ...
        if messages:
            ...
            logger.info(
                f"Memory Save Tool called with summary: '{interaction_summary}'"
            )
    ...
    def _run(self) -> str:
        ...
        try:
            ...
            if stats.get("status") == "success":
                ...
                response += (
                    f"• Redis Status: {stats_data.get('redis_status', 'Unknown')}\n"
                )
                response += (
                    f"• MongoDB Status: {stats_data.get('mongodb_status', 'Unknown')}\n"
                )
                response += f"• ChromaDB Status: {stats_data.get('chromadb_status', 'Unknown')}\n"
                response += (
                    f"• Working Memory Items: {stats_data.get('redis_keys', 0)}\n"
                )
                response += (
                    f"• User Profile Rules: {stats_data.get('mongodb_rules', 0)}\n"
                )
                response += (
                    f"• Long-term Archives: {stats_data.get('chromadb_documents', 0)}\n"
                )
                response += (
                    f"• Memory Health: {stats_data.get('overall_health', 'Unknown')}"
                )
