...
def mcp_get_context(query: str, include_long_term: bool = True) -> dict:
    ...
    return {
        "status": "success",
        "context": {"short_term": [], "profile": {}, "long_term": []},
    }
...
class LangchainMemoryContextTool(AsyncTool):
    ...
    def _run(self, query: str = "", include_long_term: bool = True) -> str:
        logger.info(
            f"Memory Context Tool called with query: '{query}', include_long_term: {include_long_term}"
        )
        try:
            result = mcp_get_memory_context(query)
            if result:
                logger.info(
                    f"Memory Context Tool: Found context (length: {len(str(result))})"
                )
    ...
    def _run(self, rule_text: str) -> str:
        ...
        try:
            ...
            if result.get("success"):
                ...
                return (
                    f"Successfully added permanent rule to your profile: '{rule_text}'"
                )
            else:
                error_msg = result.get("error", "Unknown error")
