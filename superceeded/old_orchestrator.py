...
def analyze_tool_needs(user_message: str) -> Dict[str, bool]:
    ...
    if any(
        pattern in user_message_lower for pattern in temporal_patterns + info_patterns
    ):
    ...
    is_meta_conversation = any(
        word in user_message_lower
        for word in ["logs", "system", "agent", "tool", "orchestrator"]
    )
    if (
        any(pattern in user_message_lower for pattern in flutter_patterns)
        and not is_meta_conversation
    ):
        tools["rag"] = True
    ...
    if any(pattern in user_message_lower for pattern in git_patterns):
        tools["git"] = True
...
def get_workflow_recommendations(user_message: str) -> Dict[str, bool]:
    ...
    return {"naming_conventions": False, "refactor": False, "test": False}
...
async def execute_simple_request(
    ...
    try:
        ...
        # Log the raw response from Qwen3
        ...
        try:
            ...
            save_result = mcp_save_interaction(
                interaction_messages, {"type": "simple_conversation"}
            )
            if save_result.get("status") == "success":
                logger.info(
                    f"Interaction saved to memory: {save_result.get('interaction_id', 'unknown')}"
                )
            else:
                logger.warning(
                    f"Failed to save interaction: {save_result.get('error')}"
                )
        except Exception as e:
...
async def execute_complex_request(
    ...
    try:
        ...
        if tool_recommendations.get("memory"):
            try:
                ...
                if memory_result and "No relevant context" not in memory_result:
                    context_info += f"\n--- MEMORY CONTEXT ---\n{memory_result}\n"
                    logger.info(
                        f"\033[34mMemory tool result: {memory_result[:200]}...\033[0m"
                    )
                else:
        ...
        if tool_recommendations.get("rag"):
            try:
                ...
                if flutter_result and "No relevant documentation" not in flutter_result:
                    ...
                    logger.info(
                        f"\033[33mFlutter docs result: {flutter_result[:200]}...\033[0m"
                    )
                else:
                    logger.info(
                        "\033[33mFlutter docs: No relevant documentation found\033[0m"
                    )
                ...
                if code_result and "No relevant code" not in code_result:
                    context_info += f"\n--- CODE EXAMPLES ---\n{code_result}\n"
                    logger.info(
                        f"\033[33mCode search result: {code_result[:200]}...\033[0m"
                    )
                else:
        ...
        if tool_recommendations.get("web_search"):
            try:
                ...
                if web_result and "No search results" not in web_result:
                    context_info += f"\n--- WEB SEARCH RESULTS ---\n{web_result}\n"
                    logger.info(
                        f"\033[32mWeb search result: {web_result[:200]}...\033[0m"
                    )
                else:
        ...
        logger.info(
            f"Qwen3 completed complex request. Response length: {len(final_response)}"
        )
        ...
        try:
            ...
            save_result = mcp_save_interaction(
                interaction_messages, {"type": "complex_conversation"}
            )
            if save_result.get("status") == "success":
                logger.info(
                    f"Interaction saved to memory: {save_result.get('interaction_id', 'unknown')}"
...
async def execute_agent_request(
    ...
    if needs_tools:
        ...
        return await execute_complex_request(
            messages, user_message, requested_model_name, tool_recommendations
        )
    else:
        logger.info(f"Executing simple request without tools")
        return await execute_simple_request(
            messages, user_message, requested_model_name
        )
