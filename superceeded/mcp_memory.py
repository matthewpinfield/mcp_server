...
# Memory tier paths
CHROMA_TIER3_PATH = os.path.join(
    SSD_BASE_DATA_PATH, "tier3_memory_db"
)  # Tier 3 (SSD - fast)
CHROMA_NAS_PATH = os.path.join(
    NAS_BASE_DATA_PATH, "memory_vector_db"
)  # NAS archive (slow)
...
class MemorySystem:
    ...
    def update_permanent_rule(
        self, rule_id: str, new_rule: str, new_category: str = None
    ) -> Dict:
        """
        ...
        try:
            ...
            update_data = {
                ...
                "category": (
                    new_category
                    if new_category
                    else existing_rule.get("category", "general")
                ),
    ...
    def add_correction(
        self, ai_response: str, user_correction: str, topic: str = None
    ) -> Dict:
        """
    ...
    def _search_long_term_memory(self, query: str, n_results: int = 5) -> List[Dict]:
        ...
        try:
            ...
            results = self.tier3_memory.query(query_texts=[query], n_results=n_results)
...
def mcp_update_permanent_rule(
    rule_id: str, new_rule: str, new_category: str = None
) -> Dict:
...
def mcp_add_correction(
    ai_response: str, user_correction: str, topic: str = None
) -> Dict:
...
class MemoryContextSchema(BaseModel):
    query: str = Field(
        description="Optional query to focus the memory search. Leave empty to get general context."
    )
    include_long_term: bool = Field(
        description="Whether to include semantic search of long-term memories",
        default=True,
    )
...
class MemorySaveSchema(BaseModel):
    messages: List[Dict[str, str]] = Field(
        description="List of message objects like [{'role': 'user', 'content': 'text'}]"
    )
    tags: Optional[Dict[str, str]] = Field(
        description="Optional tags for categorization like {'domain': 'programming', 'language': 'python'}",
        default=None,
    )
...
class LangchainMemoryContextTool(AsyncTool):
    ...
    description: str = (
        "Retrieves conversation context from the 3-tier memory system (Redis short-term, MongoDB profile, ChromaDB long-term). Use this to recall previous conversations, user preferences, and relevant semantic memories."
    )
    ...
    def _run(self, query: str = "", include_long_term: bool = True) -> str:
        ...
        try:
            ...
            if result["status"] == "success":
                ...
                summary += (
                    f"• Short-term: {len(context['short_term'])} recent interactions\n"
                )
                summary += f"• Profile: {len(context['profile'].get('rules', []))} rules, {len(context['profile'].get('preferences', {}))} preferences\n"
                summary += (
                    f"• Long-term: {len(context['long_term'])} semantic matches\n\n"
                )
...
class LangchainMemorySaveTool(LangchainBaseTool):
    ...
    description: str = (
        "Saves conversation messages to memory. ONLY use this to store actual chat messages for recall. For user preferences or rules, use add_permanent_rule instead. Input requires 'messages' as a list of message objects like [{'role': 'user', 'content': 'text'}]."
    )
    args_schema: Type[BaseModel] = MemorySaveSchema

    def _run(
        self, messages: List[Dict[str, str]], tags: Optional[Dict[str, str]] = None
    ) -> str:
...
class LangchainMemoryRuleTool(LangchainBaseTool):
    ...
    description: str = (
        "Adds a permanent rule or preference to user profile. USE THIS for user preferences like 'I prefer detailed explanations' or behavior requests. Input: rule='text of the rule/preference', category='preference'."
    )
...
class LangchainMemoryStatsTool(LangchainBaseTool):
    ...
    description: str = (
        "Gets diagnostic information about the memory system status and usage statistics."
    )
    ...
    def _run(self) -> str:
        ...
        try:
            ...
            if result["status"] == "success":
                ...
                summary += (
                    f"• Storage: {stats['chromadb'].get('storage_path', 'unknown')}"
                )
                return summary
            else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Test add rule
        rule_result = ms.add_permanent_rule(
            "Always explain Python concepts clearly", "programming"
        )
