...
class MemorySystem:
    ...
    def save_interaction(self, messages: List[Dict], tags: Optional[Dict] = None) -> Dict:
        ...
        try:
            ...
            if self.tier3_memory is not None:
                ...
                results["chromadb"] = "unavailable"
                logger.warning("ChromaDB not available for saving interaction")
        ...
        try:
            ...
            if self.redis_client is not None:
                try:
                    ...
                    # Redis keys() returns a list when decode_responses=True
    ...
    def _search_long_term_memory(self, query: str, n_results: int = 5) -> List[Dict]:
        ...
        try:
            # Check if tier3_memory is available
            if self.tier3_memory is None:
                logger.warning("ChromaDB Tier 3 not available, returning empty results")
                return []
...
class LangchainFlutterDocTool(AsyncTool):
    ...
    def _run(self, query: str, max_results: int = 10) -> str:
        ...
        try:
            ...
            response = requests.post(RAG_SERVER_ENDPOINT, params={"query": query, "limit": max_results}, timeout=REQUEST_TIMEOUT)
