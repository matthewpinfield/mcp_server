...
class GitHubRepoSearchSchema(BaseModel):
    ...
    language: Optional[str] = Field(
        description="Programming language filter", default=None
    )
...
class GitHubReleaseSchema(BaseModel):
    repository: str = Field(
        description="Repository in format 'owner/repo' (e.g., 'flutter/flutter')"
    )
    max_results: int = Field(description="Maximum number of results", default=5)
...
class LangchainGitHubRepoSearchTool(AsyncTool):
    ...
    description: str = (
        "Searches GitHub for repositories based on query, language, and other criteria. Useful for finding projects, libraries, or examples."
    )
    args_schema: Type[BaseModel] = GitHubRepoSearchSchema

    def _run(
        self, query: str, language: Optional[str] = None, max_results: int = 5
    ) -> str:
        logger.info(
            f"GitHub Repo Search: query='{query}', language='{language}', max_results={max_results}"
        )
        try:
            ...
            summary = (
                f"GitHub repository search results for '{query}':\n\n"
                + "\n".join(results)
            )
...
class LangchainGitHubIssuesTool(AsyncTool):
    ...
    description: str = (
        "Searches for issues in a specific GitHub repository. Useful for finding bugs, feature requests, or project status."
    )
    args_schema: Type[BaseModel] = GitHubIssueSearchSchema

    def _run(self, repository: str, state: str = "open", max_results: int = 5) -> str:
        logger.info(
            f"GitHub Issues Search: repo='{repository}', state='{state}', max_results={max_results}"
        )
        try:
            ...
            for i, issue in enumerate(issues, 1):
                ...
                updated = (
                    issue.get("updated_at", "").split("T")[0]
...
class LangchainGitHubReleasesTool(AsyncTool):
    ...
    description: str = (
        "Gets recent releases and version information for a GitHub repository. Useful for tracking updates and changelog."
    )
    ...
    def _run(self, repository: str, max_results: int = 5) -> str:
        ...
        try:
            ...
            for i, release in enumerate(releases, 1):
                ...
                published = (
                    release.get("published_at", "").split("T")[0]
