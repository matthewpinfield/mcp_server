...
class GitStatusSchema(BaseModel):
    path: str = Field(description="Path to git repository (default: current directory)", default=".")
...
class GitBranchSchema(BaseModel):
    ...
    branch_name: Optional[str] = Field(description="Branch name (required for create/checkout/delete)", default=None)
...
class LangchainGitStatusTool(AsyncTool):
    name: str = "git_status"
    description: str = "Get git repository status showing staged, modified, and untracked files."
...
class LangchainGitDiffTool(AsyncTool):
    ...
    def _run(self, path: str = ".", file_path: Optional[str] = None, staged: bool = False) -> str:
        logger.info(f"Git Diff Tool: path='{path}', file='{file_path}', staged={staged}")
        try:
            os.chdir(path)
...
class LangchainGitCommitTool(AsyncTool):
    ...
    def _run(self, path: str = ".", message: str = "", add_all: bool = False) -> str:
        ...
        try:
            ...
            return f"Commit successful: {commit_hash}\nMessage: {message}\n{result.stdout}"
...
class LangchainGitBranchTool(AsyncTool):
    ...
    def _run(self, path: str = ".", action: str = "list", branch_name: Optional[str] = None) -> str:
        logger.info(f"Git Branch Tool: path='{path}', action='{action}', branch='{branch_name}'")
        try:
            os.chdir(path)
