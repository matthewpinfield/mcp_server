...
class BuildCommandSchema(BaseModel):
    command: str = Field(
        description="Build command: 'build', 'test', 'run', 'clean', 'install', 'detect'"
    )
    directory: str = Field(description="Project directory", default=".")
    options: Optional[str] = Field(
        description="Additional command options", default=None
    )


class PackageSearchSchema(BaseModel):
    ecosystem: str = Field(
        description="Package ecosystem: 'npm', 'pypi', 'pub', 'crates', 'maven', 'rubygems'"
    )
...
class LangchainBuildCommandTool(AsyncTool):
    ...
    description: str = (
        "Execute build commands for various project types (Flutter, Node.js, Python, Rust, Go, Maven, Gradle). Supports build, test, run, clean, and install operations."
    )
    args_schema: Type[BaseModel] = BuildCommandSchema

    def _run(
        self, command: str, directory: str = ".", options: Optional[str] = None
    ) -> str:
        logger.info(
            f"Build Command Tool: command='{command}', directory='{directory}', options='{options}'"
        )
        ...
        try:
            ...
            return self._execute_build_command(
                project_path, project_type, command, options
            )
...
class LangchainPackageSearchTool(AsyncTool):
    ...
    description: str = (
        "Search for packages/libraries across different ecosystems (npm, PyPI, pub.dev, crates.io, etc.)"
    )
    args_schema: Type[BaseModel] = PackageSearchSchema

    def _run(self, ecosystem: str, query: str, max_results: int = 5) -> str:
        logger.info(
            f"Package Search: ecosystem='{ecosystem}', query='{query}', max_results={max_results}"
        )
...
class LangchainDateTimeTool(AsyncTool):
    ...
    description: str = (
        "Get the current date and time information. Use this when user asks about today's date, current time, what day it is, etc."
    )
    ...
    def _run(self) -> str:
        ...
        try:
            ...
            # Simple, concise date/time information
            result = f"Current time: {now.strftime('%I:%M %p on %A, %B %d, %Y')}"
