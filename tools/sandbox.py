...
LANGUAGE_CONFIG = {
    ...
    "typescript": {
        ...
        "cmd_template": [
            "sh",
            "-c",
            "npm install -g typescript && tsc {filename} --outDir /tmp && node /tmp/{basename}.js",
        ],
        "timeout": 45,
    },
    "java": {
    ...
    "rust": {
        ...
        "cmd_template": [
            "sh",
            "-c",
            "rustc {filename} -o /tmp/program && /tmp/program",
        ],
        "timeout": 60,
    },
    "php": {
    ...
    "flutter": {
        ...
        "cmd_template": [
            "sh",
            "-c",
            "flutter create --template=console /tmp/flutter_app && cp {filename} /tmp/flutter_app/lib/main.dart && cd /tmp/flutter_app && flutter run --device-id=flutter-tester",
        ],
        "timeout": 120,
    },
}
...
def execute_code_in_docker(
    code: str, language: str, timeout: int = 30, stdin_input: str = ""
) -> Dict:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=config["file_ext"], delete=False
        ) as f:
            f.write(code)
            temp_file = f.name
        ...
        for part in config["cmd_template"]:
            cmd.append(
                part.format(
                    filename=f"/code/{filename}", basename=basename, classname=classname
                )
            )
    ...
    try:
        ...
        return {
            ...
            "stats": {
                ...
                "docker_version": (
                    docker_info.get("ServerVersion", "unknown")
                    if docker_available
                    else None
                ),
        ...
        return {"status": "error", "error": f"Failed to get sandbox stats: {str(e)}"}
    ...
    def _run(self, tool_input: str) -> str:
        ...
        try:
            result = execute_code_in_docker(
                code, language.lower(), timeout, stdin_input
            )
...
class SandboxStatsTool(AsyncTool):
    ...
    description: str = (
        "Get real-time information about the multi-language sandbox system status, available languages, and Docker configuration."
    )
        ...
        try:
            ...
            if result["status"] == "success":
                ...
                if stats["docker_available"]:
                    response += (
                        f"🔧 Docker Version: {stats.get('docker_version', 'unknown')}\n"
                    )
                    ...
                    response += (
                        f"📚 **Supported Languages** ({stats['total_languages']}):\n"
                    )
