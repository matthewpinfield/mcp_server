...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info(
            f" Flutter Docs DB: {len(flutter_table)} docs (official docs + Firebase)"
        )
        ...
        for result in results:
            formatted_result = {
                ...
                "metadata": {
                    ...
                    if k not in ["text", "_distance", "vector"]
                },
...
async def search_flutter_docs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Documentation search failed: {str(e)}"
        )
