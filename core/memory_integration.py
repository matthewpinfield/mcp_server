...
def build_master_prompt_with_memory(base_prompt, user_message: str):
    ...
    try:
        ...
        if memory_result.get("status") != "success":
            logger.warning(
                f"Memory context retrieval failed: {memory_result.get('error', 'unknown')}"
            )
            return base_prompt
        ...
        # Add user profile (Tier 2: Rules)
        if profile:
            rules = profile.get("rules", [])
            additional_rules = profile.get("rules", {})

            if rules:
                rules_text = "\n".join(
                    [f"- {rule.get('rule', rule)}" for rule in rules[:5]]
                )
                memory_sections.append(f"**User Rules:**\n{rules_text}")

            if additional_rules and isinstance(additional_rules, dict):
                rules_text = "\n".join(
                    [f"- {k}: {v}" for k, v in additional_rules.items() if k != "_id"]
                )
                if rules_text:
                    memory_sections.append(f"**Additional Rules:**\n{rules_text}")
        ...
        if memory_sections:
            ...
            original_template = (
                ...
                if hasattr(base_prompt, "template")
                else str(base_prompt)
            )
...
Use this memory context to provide personalized, consistent responses that respect user rules and learn from past interactions."""

            # Create new PromptTemplate with enhanced content
            enhanced_prompt = PromptTemplate(
                input_variables=(
                    base_prompt.input_variables
                    if hasattr(base_prompt, "input_variables")
                    else ["tools", "tool_names", "agent_scratchpad", "input"]
                ),
                template=enhanced_template,
            )

            logger.info(
                f"Enhanced prompt with memory context: {len(memory_sections)} sections"
            )
