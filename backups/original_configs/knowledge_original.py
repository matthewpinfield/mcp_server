    ...
    try:
        if action == "save_rule":
            ...
            if result.get("status") == "success":
                ...
                return (
                    f"❌ Memory search failed: {result.get('error', 'Unknown error')}"
                )
            ...
            if result.get("status") == "success":
                ...
                return (
                    f"❌ Failed to list rules: {result.get('error', 'Unknown error')}"
                )
            ...
            if result.get("status") == "success":
                ...
                return (
                    f"❌ Failed to delete rule: {result.get('error', 'Unknown error')}"
                )
            ...
            if result.get("status") == "success":
                ...
                return (
                    f"❌ Failed to update rule: {result.get('error', 'Unknown error')}"
                )
    ...
    def save_interaction(
        ...
        try:
            ...
            if self.redis_client is not None:
                ...
                logger.warning(
                    "Redis (Tier 1) not available - conversation not saved properly"
                )
