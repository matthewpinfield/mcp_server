...
def process_slash_command(
    ...
    try:
        if action == "save_rule":
            ...
            # Use timestamp-based conversation ID for user memory
            conversation_id = f"user_memory_{int(datetime.now().timestamp())}"
            result = mcp_save_interaction(
                conversation_id, messages, {"type": "user_memory"}
            )
            if result.get("status") == "success":
...
def mcp_save_interaction(
    ...
    return get_memory_system().save_interaction(conversation_id, new_messages, tags)
