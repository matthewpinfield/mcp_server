...
from tools.knowledge import process_slash_command, mcp_save_interaction
...
from tools.all_tools import (
    ...
    LangchainMemoryContextTool,  # The fixed one for memories
    LangchainMemoryListRulesTool,  # The new one for rules
...
async def orchestrate_request(
    ...
    tools = [
        ...
        LangchainFlutterDocTool(),
        LangchainCodeSearchTool(),
        LangchainMemoryContextTool(),  # Correct, memory-only tool
        LangchainMemoryListRulesTool(),  # Correct, rule-only tool
        LangchainDateTimeTool(),
        LangchainMemoryStatsTool(),
        LangchainMemoryCorrectionTool(),
        LangchainMemorySaveTool(),
...
"""
    ...
    # The ONLY history the agent gets is the intelligent summary from Gemma.
    if (
        formatted_digest
        and "offline" not in formatted_digest
        and "No relevant memories" not in formatted_digest
    ):
        # We add this single, powerful summary to the history.
        chat_history.append(
            AIMessage(content=f"[Relevant Conversation Summary]: {formatted_digest}")
        )
    ...
    try:
        ...
        try:
            ...
            save_result = mcp_save_interaction(conversation_id, new_messages)
            if save_result.get("status") == "success":
                logger.info(f"Memory save completed for conversation {conversation_id}")
            else:
